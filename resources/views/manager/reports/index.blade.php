    ...
    <x-slot name="header">
        <div class="flex flex-col xl:flex-row items-start xl:items-center justify-between gap-6">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <p class="text-xs font-semibold uppercase tracking-wider text-amber-600">Team Analytics</p>
                    <h2 class="text-2xl font-bold text-gray-900 leading-tight">Team Reports</h2>
                    <p class="text-sm text-gray-500 mt-0.5">Performance metrics and insights for your department</p>
                </div>
            </div>
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-2">
                <div class="flex flex-wrap gap-1.5">
                    @foreach([
                        ...
                        <a href="{{ route('manager.reports.index', array_merge(request()->except(['type', 'page']), ['type' => $tabType])) }}"
            ...
            <div class="flex gap-3">
                <a href="{{ route('manager.reports.export', request()->query()) }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-emerald-600 to-green-600 text-sm font-medium text-white shadow-md shadow-emerald-500/25 hover:shadow-lg hover:-translate-y-0.5 transition">
                                    ...
                                    <div class="flex items-center justify-between p-4 rounded-xl border border-gray-100 hover:border-indigo-200 hover:bg-indigo-50/50 transition">
                                        <div class="flex items-center gap-3 min-w-0">
                                            ...
                                            <div class="min-w-0">
                                                ...
                                                <p class="text-xs text-gray-500">{{ $task->reviewer?->name ?? 'No reviewer' }} &middot; {{ $task->created_at?->format('M d, Y') }}</p>
                                            </div>
                                        </div>
            ...
            @if($type === 'employee' && isset($performance))
                <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                    ...
                    <div class="p-6">
                        ...
                        <p class="text-sm text-gray-500 mb-6">{{ $performance['employee']->department?->name ?? 'Employee' }}</p>
                ...
                <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                    ...
                    <div class="p-6">
                        <div class="text-center py-12">
                            ...
                            <p class="text-sm font-semibold text-gray-700">Select a team member from the filter above</p>
                            <p class="text-xs text-gray-500 mt-1">Choose an employee to view their individual performance report</p>
                        </div>
                    </div>
                </div>
            @endif
            ...
            {{-- DEPARTMENT TAB --}}
            @if($type === 'department' && isset($departmentReport))
                <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                    <div class="h-1.5 bg-gradient-to-r from-sky-500 to-blue-600"></div>
                    <div class="p-6">
                        <h3 class="text-lg font-bold text-gray-900 mb-1">{{ $departmentReport['department']->name }}</h3>
                        <p class="text-sm text-gray-500 mb-6">Department Performance</p>

                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                            @include('partials.metric-card', ['label' => 'Members', 'value' => $departmentReport['employees'], 'color' => 'gray'])
                            @include('partials.metric-card', ['label' => 'Total Tasks', 'value' => $departmentReport['total'], 'color' => 'gray'])
                            @include('partials.metric-card', ['label' => 'Completed', 'value' => $departmentReport['completed'], 'color' => 'green'])
                            @include('partials.metric-card', ['label' => 'Completion Rate', 'value' => $departmentReport['completion_rate'].'%', 'color' => 'indigo'])
                            @include('partials.metric-card', ['label' => 'In Progress', 'value' => $departmentReport['in_progress'], 'color' => 'blue'])
                            @include('partials.metric-card', ['label' => 'Under Review', 'value' => $departmentReport['under_review'], 'color' => 'yellow'])
                            @include('partials.metric-card', ['label' => 'Changes Requested', 'value' => $departmentReport['changes_requested'], 'color' => 'red'])
                            @include('partials.metric-card', ['label' => 'Overdue', 'value' => $departmentReport['overdue'], 'color' => 'purple'])
                        </div>

                        <div class="space-y-3">
                            @forelse($departmentReport['tasks']->take(50) as $task)
                                <div class="flex items-center justify-between p-4 rounded-xl border border-gray-100 hover:border-sky-200 hover:bg-sky-50/50 transition">
                                    <div class="flex items-center gap-3 min-w-0">
                                        <span class="flex-shrink-0 w-10 h-10 rounded-lg bg-sky-500/10 flex items-center justify-center text-xs font-bold text-sky-600">{{ $task->code ?? $task->id }}</span>
                                        <div class="min-w-0">
                                            <p class="text-sm font-semibold text-gray-900 truncate">{{ $task->title }}</p>
                                            <p class="text-xs text-gray-500">{{ $task->assignees->pluck('name')->implode(', ') ?: 'Unassigned' }}</p>
                                        </div>
                                    </div>
                                    <div class="flex items-center gap-3 flex-shrink-0">
                                        <span class="hidden sm:inline text-xs text-gray-500">{{ $task->updates_count }} updates</span>
