        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-500">Stay updated on your team's tasks and reviews</p>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                ...
                <div class="divide-y divide-gray-100">
                    @forelse($notifications as $notification)
                        <div class="p-5 flex items-start gap-4 hover:bg-gray-50 transition-colors {{ $notification->read_at ? 'opacity-70' : 'bg-amber-50/30' }}">
                            <div class="mt-1 shrink-0">
                                @if($notification->read_at)
                                    <div class="w-2.5 h-2.5 rounded-full bg-gray-300"></div>
                                @else
                                    <div class="w-2.5 h-2.5 rounded-full bg-amber-500 shadow-sm shadow-amber-300"></div>
                                @endif
                            </div>
                            <div class="flex-1 min-w-0">
                                <div class="flex items-start justify-between gap-3">
                                    ...
                                    @if(!$notification->read_at)
                                        <span class="shrink-0 inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold bg-amber-100 text-amber-800">New</span>
                                    @endif
                                </div>
                                <div class="flex items-center gap-3 mt-3">
                                    <span class="text-xs text-gray-400">{{ $notification->created_at->diffForHumans() }}</span>
                                    @if($notification->data['task_id'] ?? null)
                                        <a href="{{ route('manager.team-tasks.show', $notification->data['task_id']) }}" class="inline-flex items-center gap-1 text-xs font-semibold text-indigo-600 hover:text-indigo-500 transition">
                    ...
                    @empty
                        <div class="p-16 text-center">
                            ...
                            <p class="text-sm text-gray-400 mt-1">You'll see updates about your team's tasks here</p>
                        </div>
                    @endforelse
                </div>
            </div>
