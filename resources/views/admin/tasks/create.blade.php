        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-500">Assign a new task to your team</p>
                </div>
            </div>
            <a href="{{ route('admin.tasks.index') }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-indigo-600 to-violet-600 text-sm font-medium text-white shadow-md shadow-indigo-500/25 hover:shadow-lg hover:shadow-indigo-500/30 hover:-translate-y-0.5 transition">
                ...
                <div class="p-8">
                    <form method="POST" action="{{ route('admin.tasks.store') }}">
                            ...
                            <div class="space-y-4 pl-9">
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <select id="status" name="status" class="mt-1 border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-xl shadow-sm block w-full" required>
                                            @foreach(\App\Enums\TaskStatus::cases() as $status)
                                                <option value="{{ $status->value }}" {{ old('status', 'new') === $status->value ? 'selected' : '' }}>{{ $status->label() }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div>
                                        <x-input-label for="priority" value="Priority" />
                                        <select id="priority" name="priority" class="mt-1 border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-xl shadow-sm block w-full" required>
                        ...
                        <div class="mb-8">
                            ...
                            <div class="space-y-4 pl-9">
                                ...
                                <div>
                                    ...
                                    <select id="parent_id" name="parent_id" class="mt-1 border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-xl shadow-sm block w-full">
                                        <option value="">None</option>
                                        @foreach($parentTasks as $pt)
                                            <option value="{{ $pt->id }}" {{ old('parent_id') == $pt->id ? 'selected' : '' }}>{{ $pt->title }}</option>
                                        @endforeach
                                    </select>
                                    <p class="text-xs text-gray-500 mt-1">Optionally link this task as a subtask.</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="space-y-4 pl-9">
                                @include('partials.assignee-reviewer-picker')
                            </div>
                        </div>

                        <div class="flex items-center justify-end gap-3 mt-8 pt-6 border-t border-gray-100">
                            <a href="{{ route('admin.tasks.index') }}" class="inline-flex items-center px-4 py-2.5 text-sm font-medium text-gray-600 hover:text-gray-900 rounded-xl border border-gray-300 hover:border-gray-400 transition">Cancel</a>
