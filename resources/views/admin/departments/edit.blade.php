                    ...
                    <form method="POST" action="{{ route('admin.departments.update', $department) }}">
                        ...
                        <div class="space-y-5">
                            <div>
                                ...
                                <x-text-input id="name" name="name" type="text" class="mt-1 block w-full rounded-xl" :value="old('name', $department->name)" required />
                                <x-input-error :messages="$errors->get('name')" class="mt-2" />
                            </div>

                            <div>
                                <x-input-label for="description" value="Description" />
                                <textarea id="description" name="description" rows="3" class="mt-1 border-gray-300 focus:border-sky-500 focus:ring-sky-500 rounded-xl shadow-sm block w-full">{{ old('description', $department->description) }}</textarea>
                                <x-input-error :messages="$errors->get('description')" class="mt-2" />
                            </div>

                            <div class="flex items-center gap-3 px-4 py-3 rounded-xl {{ $department->is_active ? 'bg-emerald-50 border-emerald-100' : 'bg-gray-50 border-gray-200' }} border">
                                <input id="is_active" name="is_active" type="checkbox" value="1" {{ old('is_active', $department->is_active) ? 'checked' : '' }} class="rounded border-emerald-300 text-emerald-600 focus:ring-emerald-500 w-4 h-4" />
